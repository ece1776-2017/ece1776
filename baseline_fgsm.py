...
def mnist_tutorial(train_start=0, train_end=60000, test_start=0,
    ...
    #
    # let's generate JSMA examples
    #
    jsma = SaliencyMapMethod(model_2, sess=sess)
    jsma_params = {'theta': 1., 'gamma': 0.1,
                   'clip_min': 0., 'clip_max': 1.,
                   'y_target': None}
    adv_x_jsma = jsma.generate(x, **jsma_params)
    # probably no need to backprop_through_attack
    preds_2_jsma = model_2(adv_x_jsma)

    # DON'T WANT TO TRAIN on JSMA adv examples yet
    ...
    def evaluate_2():
        ...
        eval_params = {'batch_size': batch_size}
        accuracy = model_eval(sess, x, y, preds_2_jsma, X_test,
                              Y_test, args=eval_params)
